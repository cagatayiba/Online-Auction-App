...
@Component
public class ProductBuyValidation implements Validation<ProductBuyValidation.Context> {
    ...
    private static class ProductStateValidationStep extends ValidationStep<Context> {
        @Override
        public void validate(Context context) {
            checkCondition(context.product.getState().isUpdatable(), ErrorMessage.PRODUCT_NOT_BOUGHT);
            checkNext(context);
        }
    }
